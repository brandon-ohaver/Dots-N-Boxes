...
class Board extends React.Component {
    ...
    const suffixes = [" the Bot", " the Eternal", " the AI", " the Undefeated", " with the scuffed algorithm", " the Crusher", " the Rat", " the Clever", " the Pathetic", " Skull-Cracker", 
    " Raid Leader", " the Butcher", " the Brave", " the Mad", " the Foul", " the Cruel", " the Coward", " the Legend", " the Fool", " Pthe Swift", ];
    // most suffixes are from: https://shadowofwar.fandom.com/wiki/Nemesis#Titles
    // names array provided by: https://gist.github.com/ruanbekker/a1506f06aa1df06c5a9501cb393626ea
    this.redName = prompt("Enter name for red player", ""); // red player name for leaderboard
    this.blueName = names[Math.floor(Math.random() * names.length)] += suffixes[Math.floor(Math.random() * suffixes.length)]; // random name for bot
    console.log(this.redName + " vs. " + this.blueName);
    return state
  }
    ...
    makeWinMessage = (state) => { // print message when all lines are filled
      var winMessage
        if (state.numRed > state.numBlue) {
          winMessage = this.redName + " wins against " + this.blueName + "! Select a board size to start a new game."
          // leaderboardEntries.push([this.redName, state.numRed]); need a smart way to store leaderboardEntries. Possibly in a different file?
        } else if (state.numRed < state.numBlue) {
          winMessage = this.blueName + " wins against " + this.redName + "! Select a board size to start a new game."
          // leaderboardEntries.push([this.blueName, state.numBlue]); need a smart way to store leaderboardEntries. Possibly in a different file?
        } else {
